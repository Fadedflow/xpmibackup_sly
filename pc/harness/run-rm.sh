#!/usr/bin/env bash
# RootModulesHelp 桌面单测（无 Android 依赖部分）
set -e
...
mkdir -p "$OUT"
javac -encoding UTF-8 -cp "$ANDROID_JAR;$OKHTTP_W;$OKIO_W;$KOTLIN_W" -d "$OUT" \
  ...
  "$APP_SRC/comm/RootModulesHelp.java" \
  "$PC_WIN/harness/RootModulesT.java"
java -Dfile.encoding=UTF-8 -cp "$OUT;$LAYOUTLIB;$ANDROID_JAR;$OKHTTP_W;$OKIO_W;$KOTLIN_W" RootModulesT
